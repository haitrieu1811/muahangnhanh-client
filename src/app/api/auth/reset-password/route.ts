...
import { ResetPasswordReqBody } from '@/types/users.types'

export async function POST(request: Request) {
  const cookieStore = await cookies()
  const body = (await request.json()) as ResetPasswordReqBody
  try {
    const res = await usersApis.resetPasswordFromNextServerToServer(body)
