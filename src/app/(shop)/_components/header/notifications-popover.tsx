              ...
              {notifications.map((notification) => (
                <Link
                  ...
                  className={cn('flex items-center space-x-4 p-4 rounded-md duration-100', {
                    ...
                    'bg-main/10 dark:bg-main-foreground/10': true
                  })}
                >
                  <Image
                    width={50}
                  ...
                  <div className='flex-1'>
                    <h3 className='font-normal text-sm line-clamp-2'>{notification}</h3>
                    <p className='text-sm text-muted-foreground'>1 giờ trước.</p>
                  </div>
                  {true && <div className='size-2 rounded-full bg-highlight' />}
                </Link>
              ))}
            </div>
